...
def download_esa_worldcover_direct(bounds, output_path):
    """
    Download ESA WorldCover tiles and merge if area spans multiple tiles
    """
    try:
        ...
        print(f"Area spans {len(tiles_needed)} tile(s):")
        for tile_name, _, _ in sorted(tiles_needed):
            print(f"  - {tile_name}")
        ...
        for idx, (tile_name, _, _) in enumerate(sorted(tiles_needed), 1):
            tile_path = os.path.join(temp_dir, f"{tile_name}.tif")
            
            if os.path.exists(tile_path) and os.path.getsize(tile_path) > 1000:
                print(f"  ✅ Tile {idx}/{len(tiles_needed)} already exists: {tile_name}")
            ...
            for i, url in enumerate(urls):
                try:
                    print(f"  📥 Tile {idx}/{len(tiles_needed)}: {tile_name} (URL {i+1}/{len(urls)})...")
                    ...
                    if response.status_code == 200:
                        ...
                        if os.path.getsize(tile_path) > 1000:
                            print(f"    ✅ Downloaded ({os.path.getsize(tile_path) / 1024 / 1024:.1f} MB)")
                            downloaded_tiles.append(tile_path)
                            downloaded = True
                            break
                        else:
        ...
        print(f"\n✅ Downloaded {len(downloaded_tiles)}/{len(tiles_needed)} tiles")
        
        # If only one tile, just use it
        if len(downloaded_tiles) == 1:
            shutil.copy(downloaded_tiles[0], output_path)
            print(f"Single tile - copied directly")
            return True
        
        # Multiple tiles - merge them
        print(f"\n🔗 Merging {len(downloaded_tiles)} tiles...")
        ...
        print(f"✅ Successfully merged into single raster")
        return True
    ...
    try:
        ...
        with rasterio.open(input_path) as src:
            ...
            def process_block(i, j):
                ...
                return 60  # Default to bare/sparse vegetation
        ...
        with rasterio.open(output_path) as src:
            ...
            if 0 in unique_out:
                print("⚠️  WARNING: Found class 0 in output!")
            else:
                print("✅ No class 0 found - aggregation successful!")
...
def print_landuse_info(landuse_path):
    ...
    try:
        with rasterio.open(landuse_path) as src:
            ...
            file_size_mb = os.path.getsize(landuse_path) / (1024 * 1024)
