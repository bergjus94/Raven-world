...
from structure_parameter_tables import (  # noqa: E402
    PARAM_INFO, STRUCTURES, load_bounds, fmt_num,
)
...
def render_table_panel(fig, gs_table_section, struct_key,
    ...
    # Header
    ax_header.text(0.0, 0.92, f'{struct_key}: {s["name"]}',
                   fontsize=14, weight='bold', va='top')
    ax_header.text(0.0, 0.72, f'Configuration: {s["config"]}.yaml',
    ...
    ax_header.text(0.0, 0.22, s['process_change'],
...
def render_combined(struct_key, bounds_lo, bounds_hi, bounds_init, outdir: Path):
    ...
    fp = outdir / f'{struct_key}_combined.png'
