...
class SPHYProcessor:
    ...
    def _build_global_params(self, param_or_name: str) -> List[str]:
        ...
        # ignored under OROCORR_HBV — see model_structure_decisions.md §
        # "Precipitation lapsing — data-derived approach".
        lapse = load_lapse_rates(self.config)
        if lapse is not None:
            lines += [
                f":GlobalParameter HBVEC_LAPSE_RATE   {lapse['HBVEC_LAPSE_RATE']:.5f}  "
                f"# data-derived ({lapse['source']}, n={lapse['n_cells']})",
                f":GlobalParameter HBVEC_LAPSE_UPPER  {lapse['HBVEC_LAPSE_UPPER']:.5f}  "
                f"# data-derived ({lapse['source']})",
                f":GlobalParameter HBVEC_LAPSE_ELEV   {lapse['HBVEC_LAPSE_ELEV']:.1f}  "
                f"# data-derived breakpoint",
            ]
        else:
            print(
                f"⚠️  lapse_rate.yaml not found for gauge {self.gauge_id} — "
                f"OROCORR_HBV will use Raven defaults "
    ...
    def _create_rvi_sections(self, start_date: str, end_date: str, cali_end_date: str,
        ...
        return {
            ...
            # the hydrograph; the full set of :CustomOutput directives (soil
            # storage, baseflow fluxes, snow fluxes, glacier tracers etc.) is
