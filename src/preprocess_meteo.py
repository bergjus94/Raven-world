...
class TPHiPrAnalyzer:
    ...
    def __init__(self, namelist_path: Union[str, Path], force_reprocess: bool = False) -> None:
        ...
        # Warm-up date (extend time range to include warm-up period)
        if 'warm_up_date' in self.config:
            self.warmup_date = pd.to_datetime(self.config.get('warm_up_date'))
        else:
            self.warmup_date = None
        ...
        self.logger = logging.getLogger(f'TPHiPrAnalyzer_Gauge_{self.gauge_id}')
    ...
    def _load_catchment_bounds(self) -> Optional[tuple]:
        """Return (minx, miny, maxx, maxy) of the catchment in WGS84, or None."""
        shape_template = self.config.get(
            'shape_dir',
            '01_data/topo/catchment_shapefile/catchment_shape_{gauge_id}.shp'
        )
        shape_path = Path(shape_template.format(gauge_id=self.gauge_id))
        if not shape_path.is_absolute():
            shape_path = self.main_dir / shape_path

        if not shape_path.exists():
            self.logger.warning(f"Catchment shapefile not found: {shape_path}; no clipping applied")
            return None

        extent = gpd.read_file(shape_path)
        if str(extent.crs) != 'EPSG:4326':
            extent = extent.to_crs('EPSG:4326')
        return tuple(extent.total_bounds)  # (minx, miny, maxx, maxy)
    ...
    def process(self) -> Path:
        ...
        bounds = self._load_catchment_bounds()
...
class TPHiPrGridWeightsGenerator:
    """
    ...
    def __init__(self, namelist_path: Union[str, Path], force_reprocess: bool = False) -> None:
        ...
        self.logger = logging.getLogger(f'TPHiPrGridWeightsGenerator_Gauge_{self.gauge_id}')
