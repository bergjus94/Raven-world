...
class ERA5LandAnalyzer(MeteoBase):
    """
    A class for analyzing and plotting ERA5-Land meteorological data
    """
    _logger_class_name = 'ERA5LandAnalyzer'
    ...
    def __init__(self, namelist_path: Union[str, Path], force_reprocess: bool = False) -> None:
        ...
        # ERA5-specific: meteo data directory
    ...
    def _combine_monthly_files_with_clipping(self, file_list: List[Path], variable_type: str, 
        ...
        try:
            ...
            self.logger.info(f"Combined {variable_type} dataset shape: {dict(ds.sizes)}")
    ...
    def _aggregate_to_daily(self, dataset: xr.Dataset, variable_type: str) -> Dict[str, xr.Dataset]:
        ...
        try:
            if variable_type == 'temperature':
                ...
                sample_values = dataset[pet_var].isel(time=slice(0, 24)).mean(dim=['lat', 'lon']).values
                ...
                if dataset[pet_var].attrs.get('units') == 'm':
                    ...
                    first_day_values = dataset[pet_var].isel(time=slice(0, 24)).mean(dim=['lat', 'lon']).values
                ...
                pet_stats = daily_sum[pet_var].mean(dim=['lat', 'lon'])
                ...
                if len(daily_values) > 0:
                    ...
                    if daily_values.max() > 30:
                        ...
                        mean_values = daily_mean_test[pet_var].mean(dim=['lat', 'lon']).values
    ...
    def process_geopotential_to_elevation(self) -> Optional[Path]:
        ...
        try:
            ...
            self.logger.info(f"  Dimensions: {dict(ds_geo.sizes)}")
    ...
    def _clip_to_catchment(self, dataset: xr.Dataset) -> xr.Dataset:
        ...
        try:
            ...
            buffered_bounds = self.catchment_extent.buffer(0.1).total_bounds
            self.logger.debug(f"Catchment bounds (buffered): lon [{buffered_bounds[0]:.4f}, {buffered_bounds[2]:.4f}], lat [{buffered_bounds[1]:.4f}, {buffered_bounds[3]:.4f}]")

            # After normalize_coords(), lat always increases south→north
            ...
            clipped = dataset.sel(lat=lat_slice, lon=lon_slice)

            # Verify we got data
            if clipped.lat.size == 0 or clipped.lon.size == 0:
                self.logger.error("❌ Clipping resulted in empty dataset!")
                self.logger.warning("Returning unclipped dataset")
                return dataset

            original_size = dataset.lat.size * dataset.lon.size
            clipped_size = clipped.lat.size * clipped.lon.size
    ...
    def _save_daily_files(self, daily_datasets: Dict[str, xr.Dataset]) -> List[Path]:
        ...
        if elevation_file.exists():
            try:
                ...
                if 'elevation' in elevation_dataset.data_vars:
                    # Normalize coordinate names and orientation for elevation data
                    self.logger.debug("Normalizing coordinates for elevation data...")
                    elevation_dataset = normalize_coords(elevation_dataset)
                    self.logger.debug("✅ Elevation data coordinates normalized")
                else:
        ...
        for var_name, dataset in daily_datasets.items():
            try:
                ...
                # Normalize coordinate names and orientation for meteorological data
                self.logger.debug(f"Normalizing coordinates for {var_name} before saving...")
                dataset_corrected = normalize_coords(dataset)

                # ✅ NEW: Add elevation BEFORE clipping (if available)
                elevation_added = False
                if elevation_dataset is not None:
                    try:
                        # After normalize_coords(), coords are always 'lat'/'lon'
                        meteo_lat = dataset_corrected.coords.get('lat')
                        meteo_lon = dataset_corrected.coords.get('lon')
                        elev_lat = elevation_dataset.coords.get('lat')
                        elev_lon = elevation_dataset.coords.get('lon')
                ...
                final_lat = dataset_corrected.coords.get('lat')
                final_lon = dataset_corrected.coords.get('lon')
                ...
                self.logger.debug(f"  Dimensions: {dict(dataset_corrected.sizes)}")
    ...
    def plot_spatial_overview(self, netcdf_file: Path) -> None:
        ...
        try:
            # Open the dataset
            ds = normalize_coords(xr.open_dataset(netcdf_file, chunks={'time': 100}))
            ...
            for i, var_name in enumerate(data_vars):
                ...
                if hasattr(ds, 'lon') and hasattr(ds, 'lat'):
                    im = data.plot(ax=ax, x='lon', y='lat', cmap=var_info['cmap'], add_colorbar=False)
                else:
            ...
            if has_elevation:
                ...
                # Plot elevation
                if hasattr(ds, 'lon') and hasattr(ds, 'lat'):
                    im = elevation_data.plot(ax=ax, x='lon', y='lat', cmap='terrain', add_colorbar=False)
                else:
    ...
    def debug_precipitation_values(self, netcdf_file: Path) -> None:
        ...
        try:
            ...
            ds = normalize_coords(xr.open_dataset(netcdf_file))
            ...
            spatial_mean = precip_data.mean(dim=['lat', 'lon'])
    ...
    def debug_hourly_precipitation(self, monthly_files: List[Path]) -> None:
        ...
        try:
            ...
            ds = normalize_coords(xr.open_dataset(sample_file))
            ...
            # Sample some values
            sample_values = precip_data.isel(time=slice(0, 24)).mean(dim=['lat', 'lon']).values
...
class GridWeightsGenerator(MeteoBase):
    """
    ...
    _logger_class_name = 'GridWeightsGenerator'
    ...
    def netCDF_to_GeoDataFrame(self, netcdf: xr.Dataset) -> gpd.GeoDataFrame:
        ...
        # normalize_coords() was called before this method — coords are always 'lat'/'lon'
        if 'lat' not in ds.coords or 'lon' not in ds.coords:
            raise ValueError("Cannot find lat/lon coordinates in ERA5-Land netCDF (normalize_coords should have been called first)")
        ...
        self.logger.debug("📍 Sample coordinates (first 3 points):")
    ...
    def generate(self, use_precipitation: bool = True) -> gpd.GeoDataFrame:
        ...
        xds = normalize_coords(xr.open_dataset(era5_file))
